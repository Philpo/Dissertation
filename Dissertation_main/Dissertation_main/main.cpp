...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
  ...
  sheetDataFile.open("sheet_test_data.csv", ios_base::app);
  flagDataFile.open("flag_test_data.csv",ios_base::app);
  //sheetDataFile << "Test ID, Integrator, Rows, Columns, Time Step, Average time calcSpringForce (ms), Average time external force (ms), Average time integrate (ms), Average time updating (ms), Average rendering time (ms), Average FPS, Update count" << endl;
  //flagDataFile << "Test ID, Integrator, Rows, Columns, Time Step, Average time calcSpringForce (ms), Average time external force (ms), Average time integrate (ms), Average time updating (ms), Average rendering time (ms), Average FPS, Update count" << endl;
  ...
  while (WM_QUIT != msg.message) {
    if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
      ...
      if (simulationRunningTime >= MAX_SIMULATION_TIME) {
